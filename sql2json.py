...
class SQLdb(object):
    ...
    def loaddb(self, dbfilename):
        ...
        try:
            ...
            log.critical('cant connect to SQLite db %s: %s', dbfilename, str(sys.exc_info()[1]))
            raise Exception(e)
        self.setconn(conn)
...
class SQLdump(object):
    def __init__(self, dumpfile):
        ...
        try:
            ...
            log.critical('cant load SQLite dump from %s: %s', dumpfile, str(sys.exc_info()[1]))
            raise Exception(e)
        try:
            ...
            log.critical('cant import SQLite dump %s: %s', dumpfile, str(sys.exc_info()[1]))
            raise Exception(e)
        self.conn = conn
